...
- [x] Why would you use class component over function components (removing hooks from the question)? To control state more effectively and lifecycle.
- [x] Name three lifecycle methods and their purposes. mounting componentdidmount(constructor, Render)--- update componentDidUpdate unmounting--componentWillUnmount..
- [x] What is the purpose of a custom hook?--a way to reuse stateful logic for a DRY code.
- [x] Why is it important to test our apps?--avoid bugs but it also saves time if you TDD..In the long run you get a functining product that would be fully functionable.
